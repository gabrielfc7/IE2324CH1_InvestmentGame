/**
 * IE2324 Portfolio Backend (semester + section + name)
 * Sheet columns: timestamp | semester | section | name | uid | allocations_json | cash
 */
function doPost(e){
  try{
    ...
    sheet.appendRow([new Date(), semester, section, name, uid, JSON.stringify(allocations), cash]);
...
function doGet(e){
  ...
  if(mode === 'submissions') return json({ ok:true, items: data });
  if(mode === 'leaderboard'){
...
function readRows(semester, section){
  var sheet = SpreadsheetApp.getActive().getActiveSheet();
  var values = sheet.getDataRange().getValues(); var header = values.shift();
  var idx = {
    ts: header.indexOf('timestamp'), sem: header.indexOf('semester'),
    section: header.indexOf('section'), name: header.indexOf('name'),
    uid: header.indexOf('uid'), al: header.indexOf('allocations_json'), cash: header.indexOf('cash')
  };
  return values.filter(function(r){
    ...
    return { timestamp:r[idx.ts], semester:r[idx.sem], section:r[idx.section], name:r[idx.name],
      uid:r[idx.uid], allocations: JSON.parse(r[idx.al]||'[]'), cash: Number(r[idx.cash]||0) };
  });
}
function computeLeaderboard(rows, prices, timeline, topN){
  ...
  rows.forEach(function(r){
    ...
    r.allocations.forEach(function(a){
      ...
      var p = base; factorsPerMonth.forEach(function(f){ p *= (f[a.stockId] || 1); });
      value += a.shares * p;
    });
    var key = r.name ? (r.name + ' (' + r.uid + ')') : r.uid;
    if(!users[key]) users[key] = { uid:key, semester:r.semester, section:r.section, value:0 };
    users[key].value = Math.max(users[key].value, value);
  });
  var list = Object.keys(users).map(function(k){ return users[k]; })
              .sort(function(a,b){ return b.value-a.value; }).slice(0, topN||10);
  return { months: timeline.map(function(t){ return t.month; }), finalPrices:{}, leaders:list };
}
